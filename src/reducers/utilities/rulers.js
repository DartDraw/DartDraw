...
export function updateMouseTrackers(x, y, scale, panX, panY, rulerWidth, canvasWidth, canvasHeight, gridSnapping, gridSnapInterval) {
    ...
    if (gridSnapping) {
        mouseTrackerX = (Math.round((mouseTrackerX + (panX * scale)) / gridSnapInterval / scale) * gridSnapInterval - panX) * scale;
        mouseTrackerY = (Math.round((mouseTrackerY + (panY * scale)) / gridSnapInterval / scale) * gridSnapInterval - panY) * scale;
    }
...
export function changeRulerPreset(ruler, newRulerName) {
...
export function setPixelsPerUnit(unitType) {
...
export function constrainInput(payload) {
